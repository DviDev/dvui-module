...
<div class="w-full" wire:ignore>
    ...
    <select
        ...
        @endif
        {{$attributes->class([
        "rounded p-2 border border-gray-200 mr-2 focus:border-gray-300 w-full dark:border-gray-500 dark:bg-transparent
        py-[0.30rem] ",
        "bg-transparent" => (bool)collect($attributes->get('class'))->first(fn($class) =>
        str($class)->startsWith('bg-')),
        "bg-white"
        ])}}
        {{$attributes}}>
        {{$slot}}
