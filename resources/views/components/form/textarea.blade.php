...
@php
    $wire_model_ = collect($attributes->getAttributes())->filter(function($value, $key) {
        return str($key)->contains('wire:model');
    })->first();

    $model_ = str($wire_model_)->explode('.')->first();
...
@endphp
<div class="w-full" x-data="{ model_value: $wire.{{$wire_model_}} }">
    ...
    <div class="text-right text-gray-400 text-xs flex justify-end">
        ...
        <span title="total allowed">{{$attributes->get('maxlength', 0) }}</span>
    </div>
